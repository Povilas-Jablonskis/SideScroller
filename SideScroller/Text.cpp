...
namespace Engine
{
	...
	void Text::fixPosition(UIElementBase* parent)
	{
		...
		if (parent != nullptr)
		{
			...
			{
				setPosition(0, parent->getPosition(0) + (parent->getWidth() * (positionPercents.x / 100.0f)));
				setPosition(1, parent->getPosition(1) + (parent->getHeight() * (positionPercents.y / 100.0f)));
			}
		}
		else
		{
