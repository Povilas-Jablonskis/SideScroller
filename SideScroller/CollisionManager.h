		...
		public:
			...
			void checkCollision(std::shared_ptr<Player> object, std::vector<std::shared_ptr<T2>>& colliderList)
			{
				...
				for (std::vector<std::shared_ptr<T2>>::iterator it = colliderList.begin(); it != colliderList.end(); ++it)
				{
					...
					collisionInfo = checkCollision(glm::vec4(object->getPosition(0), object->getLastPosition(1), object->getWidth(), object->getHeight()), glm::vec4((*it)->getPosition() - glm::vec2(object->getCamera(0), object->getLastCamera(1)), (*it)->getWidth(), (*it)->getHeight()));
				...
				{
					if ((*it)->getNeedsToBeDeleted() || (*it)->getFirstState() == STATE_DEAD) continue;
					collisionInfo = checkCollision(glm::vec4(object->getLastPosition(0), object->getPosition(1), object->getWidth(), object->getHeight()), glm::vec4((*it)->getPosition() - glm::vec2(object->getLastCamera(0), object->getCamera(1)), (*it)->getWidth(), (*it)->getHeight()));
			...
			void checkCollision(std::shared_ptr<Entity> object, std::vector<std::shared_ptr<T2>>& colliderList)
			{
				...
				{
					if ((*it)->getNeedsToBeDeleted() || (*it)->getFirstState() == STATE_DEAD) continue;
					collisionInfo = checkCollision(glm::vec4(object->getPosition(0), object->getLastPosition(1), object->getWidth(), object->getHeight()), glm::vec4((*it)->getPosition(), (*it)->getWidth(), (*it)->getHeight()));
					if (checkCollision(object, *it, true, collisionInfo))
						break;
				}
				for (std::vector<std::shared_ptr<T2>>::iterator it = colliderList.begin(); it != colliderList.end(); ++it)
				{
					if ((*it)->getNeedsToBeDeleted() || (*it)->getFirstState() == STATE_DEAD) continue;
					collisionInfo = checkCollision(glm::vec4(object->getLastPosition(0), object->getPosition(1), object->getWidth(), object->getHeight()), glm::vec4((*it)->getPosition(), (*it)->getWidth(), (*it)->getHeight()));
					if (checkCollision(object, *it, false, collisionInfo))
						break;
				}
			}
